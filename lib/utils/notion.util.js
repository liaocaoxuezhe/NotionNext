...
export function adapterNotionBlockMap(blockMap) {
  ...
  for (const [id, block] of Object.entries(blockMap.block || {})) {
    const unwrapped = unwrapValue(block);
    // 【修复】跳过无效的block（缺少id的block会导致react-notion-x报错）
    if (!unwrapped?.id) {
      console.warn('[adapterNotionBlockMap] 跳过无效block（缺少id）:', id);
      continue;
    }
    cleanedBlocks[id] = { value: unwrapped };
  }
