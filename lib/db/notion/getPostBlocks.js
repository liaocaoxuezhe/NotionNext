...
export async function fetchNotionPageBlocks(id, from = null, slice = 0) {
  ...
  // 3️⃣ 如果指定了 slice，则截取指定数量的 block
  if (slice > 0 && pageBlock.block) {
...
export function formatNotionBlock(block) {
  ...
  for (let i = 0; i < blocksToProcess.length; i++) {
    ...
    // 【修复】跳过无效的block（id为undefined的block会导致react-notion-x报错）
    if (!b?.value?.id) {
      console.warn('[formatNotionBlock] 删除无效block（缺少id）:', blockId)
