 ...
 * @param {string} id
 * @param {*} from
 * @param {number} slice 预览时截取的 block 数量
 */
export async function fetchNotionPageBlocks(id, from = null, slice = null) {
  ...
  // ✅ 如果指定了 slice，截断 block 实现预览（保持 recordMap 结构不变）
  if (slice && slice > 0 && pageBlock.block) {
    const blockKeys = Object.keys(pageBlock.block)
    if (blockKeys.length > slice) {
      const slicedBlock = {}
      for (const key of blockKeys.slice(0, slice)) {
        slicedBlock[key] = pageBlock.block[key]
      }
      pageBlock.block = slicedBlock
    }
  }
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  ...
  try {
    ...
    if (e?.status === 429 || e?.message?.includes('429')) {
      const delayMs = Math.pow(2, 3 - retryAttempts) * 1000 // 1s, 2s, 4s
      console.warn(`[RateLimit] 遇到429，等待 ${delayMs}ms 后重试...`)
      await delay(delayMs)
    }
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    while (attempts > 0) {
      try {
        ...
        console.warn('[Batch API异常]', err.message, `剩余重试:${attempts - 1}`)
        attempts--
        if (attempts === 0) {
          console.error('[Batch API失败] 本批次数据已丢弃:', batch)
        } else {
          await delay(Math.pow(2, 3 - attempts) * 1000) // 指数退避
        }
      }
    }
  }
